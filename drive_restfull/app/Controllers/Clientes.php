        ...
        foreach ($registro as $key => $value) {
            ...
            if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
                ...
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value["cliente_id"] . ":" . $value["llave_secreta"])) {
                    ...
                    // $db = \Config\Database::connect();
                    // $query = $db->query('SELECT pe.*, s.nombre as sexo, c.nombre as cargo
                    //     FROM persona AS pe 
                    //     INNER JOIN sexo as s ON pe.id_sexo = s.id 
                    //     INNER JOIN cargo as c ON pe.id_cargo = c.id
                    //     WHERE pe.estado=1');
                ...
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value["cliente_id"] . ":" . $value["llave_secreta"])) {
                    ...
                    if (!empty($datos)) {
                        ...
                        $validation->setRules([
                            ...
                            'id_sexo' => 'string|max_length[255]',
                            'id_tipo_doc' => 'string|max_length[255]', 
                            'num_doc' => 'string|max_length[255]',
