...
use App\Models\PorcentajeModel;
...
class Porcentajes extends Controller {
        ...
        foreach ($registro as $key => $value) {
            ...
            if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
                ...
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value["cliente_id"] . ":" . $value["llave_secreta"])) {
                    ...
                    $Planilla = new PorcentajeModel();
                    //$librodiario = $Librodiario->findAll();
                    $Planilla = $Planilla->get_porcentajes_des();
                    //var_dump($Planilla); die;
    ...
    public function total_pedido($id) {
     ...
     public function porcentajes_pago() {
        //realiza solicitud a services y le decimos que ejecute el metodo request()
        $request = \Config\Services::request();
        $validation = \Config\Services::validation();
        $headers = $request->getHeaders();
        ...
        foreach ($registro as $key => $value) {
            ...
            if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
                ...
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value["cliente_id"] . ":" . $value["llave_secreta"])) {
                    ...
                    $Librodiario = new PorcentajeModel();
                    $prendas = $Librodiario->get_porcentajes_pago();
                    if (!empty($prendas)) {
