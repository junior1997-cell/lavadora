...
function mostrar(id)
{
	$.post("../ajax/insumos.php?op=mostrar",{id : id}, function(data, status)
	{
		...
		$("#stock").val(data["Detalle"]["stock"]);
		$("#descripcion").val(data["Detalle"]["descripcion"]);
		$("#imagenmuestra").show();
		$("#imagenmuestra").attr("src","../files/insumos/"+data["Detalle"]["imagen"]);
...
function desactivar(id)
{
	bootbox.confirm("¿Está Seguro de desactivar el insumo?", function(result){
		if(result)
        {
        	$.post("../ajax/insumos.php?op=desactivar", {id : id}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function activar(id)
{
	bootbox.confirm("¿Está Seguro de activar el insumos?", function(result){
		if(result)
        {
        	$.post("../ajax/insumos.php?op=activar", {id : id}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
